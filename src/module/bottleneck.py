...
def bottleneck(input, train):
    ...
    filter = Filter()
    filter.size = [1, 3, 1]
    filter.channel = [128, 128, 256, input.get_shape()[-1]]
    filter.shape = [
    ...
    def conv(tensor, index, filter = filter):
    ...
    with tf.variable_scope('conv_00'):
        conv_00 = conv(tensor = input, index = 0)
    ...
    with tf.variable_scope('conv_01'):
        conv_01 = conv(tensor = conv_00, index = 1)
    ...
    with tf.variable_scope('conv_02'):
        conv_02 = conv(tensor = conv_01, index = 2)
